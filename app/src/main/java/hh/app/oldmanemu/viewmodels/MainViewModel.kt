...
class MainViewModel : ViewModel() {
    val homePageData: MutableLiveData<HomePageBean> by lazy {
        MutableLiveData<HomePageBean>().also {
                    ...
                    override fun onResponse(
                        ...
                        response.body()?.apply {
                            ...
                            var document = Jsoup.parse(htmlStr)
                                document.getElementsByClass("navbar navbar-expand-lg navbar-dark bg-dark").apply {
                                    if(isNotEmpty()){
                                        var baseNaviBar =get(0)
                                        var naviBar =
                                            baseNaviBar.getElementsByClass("navbar-nav mr-auto").get(0).getElementsByClass(
                                                "nav-link"
                                            )?:null
                                        ...
                                        //topic list
                                        var topiclistHtml =
                                            document.getElementsByClass("list-unstyled threadlist mb-0").get(0)
                                                .getElementsByTag("li")
                                        ...
                                        for (element in topiclistHtml) {
                                            ...
                                            var postAvatar=posterInfo.getElementsByTag("img").get(0)
                                                .attr("src")
                                            var postDetail=if(extraInfo.getElementsByClass("date text-grey hidden-sm").isNotEmpty())
                                                extraInfo.getElementsByClass("date text-grey hidden-sm").get(0).text() else ""
                                            var replyDetail=if(extraInfo.getElementsByClass("username text-grey mr-1").isNotEmpty())
                                                extraInfo.getElementsByClass("username text-grey mr-1").get(0).text() else ""
                                            var replyTime=if(extraInfo
                                                .getElementsByClass("text-grey").size>2)extraInfo
                                                .getElementsByClass("text-grey").get(2).text()
                                                else extraInfo
                                                .getElementsByClass("text-grey").get(extraInfo
                                                    .getElementsByClass("text-grey").size-1).text()
                                            var topicBean = TopicBean(
                                                ...
                                                replyTime =replyTime ,
                                                replyNum = numInfo.getElementsByClass("ml-2").get(1).text()
                                                    .replace("\"", "")
                                                    .replace(" ", ""),
                                                favNum = numInfo
                                                    .getElementsByClass("ml-2").get(2).text().replace("\"", "")
                                                    .replace(" ", "")
                                            )
                                        ...
                                        //notification num
                                        var notify=document.getElementsByClass("d-none unread badge badge-danger badge-pill")
//                                        if(notify)
//                                        homePageBean.notifyNum=
                                        //Page index
